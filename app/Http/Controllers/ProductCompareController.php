...
class ProductCompareController extends Controller
{
    public function addProductToCompare($id)
    {
        ...
        return response()->json(['message' => 'Product added for comparison'], 200);
    }

    public function removeProductToCompare($id)
    {
        $product = ProductCompare::find($id);

        $product->delete();

        return response()->json(['message' => 'Product removed from comparison', 200]);
    }
    ...
    public function getProductsToCompare()
    {
        ...
        $comparedProducts = ProductCompare::where('user_id', $userId)
            ->with(['product.image', 'product.reviews', 'product.category'])
            ->get();
